...
document.querySelectorAll("[data-template]").forEach(b=>b.onclick=()=>{start.classList.remove("show");scene.className="scene "+b.dataset.template;if(b.dataset.template==="long")makeObj("part long","Длинный Part");if(b.dataset.template==="square")makeObj("part square","Квадрат");if(b.dataset.template==="parkour"){for(let i=0;i<5;i++)makeObj("part long","Parkour")}});
...
saveProject.onclick=()=>saveModal.classList.add("show");
