...
import { games, renderHeaderUser, getSession } from "/data.js";
...
banner.src=current.banner; desc.textContent=current.desc; likes.textContent=current.likes; genre.textContent=current.slug==="parkour"?"Parkour":"Sandbox";
onSnapshot(query(collection(db,"presence"),where("gameId","==",current.id)),(snap)=>{online.textContent=snap.size;activeStat.textContent=snap.size});
